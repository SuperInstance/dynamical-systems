...
| `behavior` | Behavioral attractor discovery, approach-avoidance model, behavior prediction |

---

## Key Idea

A dynamical system evolves a state over time. This library lets you **define** such systems (continuous or discrete), **analyze** them (fixed points, stability, bifurcations, chaos), and **apply** the theory to behavioral modeling. Every algorithm runs in a single process—no external solvers needed.
...
```rust
use dynamical_systems::continuous::{LorenzSystem, rk4};
...
```rust
use dynamical_systems::discrete::{LogisticMap, iterate};
use nalgebra::DVector;
...
```rust
use dynamical_systems::fixed_points::{
    analyze_continuous_fixed_point, find_fixed_point_continuous, Stability
};
use dynamical_systems::continuous::LorenzSystem;
use nalgebra::DVector;
...
```rust
use dynamical_systems::strange_attractors::lyapunov_spectrum_continuous;
use dynamical_systems::continuous::LorenzSystem;
use nalgebra::DVector;
...
```rust
use dynamical_systems::bifurcation::logistic_bifurcation;
...
```rust
use dynamical_systems::basin::basin_1d_discrete_map;
...
10. **Behavior modeling** applies all of the above to behavioral modeling: a system's state evolves according to a dynamical system, and stable fixed points correspond to persistent behavioral patterns.
